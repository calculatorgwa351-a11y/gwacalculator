      ...
      <div class="p-8 space-y-6 max-h-[80vh] overflow-y-auto">
        ...
        >
          ...
          <div class="mt-2 text-sm font-semibold">
            Inserted: {{ importResult.inserted }} | Updated: {{ importResult.updated }} | Errors: {{ importResult.errors.length }}
          </div>
        ...
        <div v-else-if="grades.length === 0" class="text-center p-10 rounded-[2rem] bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-slate-500 dark:text-slate-400 italic">
          No grades recorded yet for this student. Use Add Grade or import a CSV file.
        </div>

        <div v-else class="space-y-4">
          <div v-for="grade in grades" :key="grade.id" class="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-700 shadow-sm">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
              <div class="flex items-center gap-3">
                <div :class="['w-11 h-11 rounded-2xl flex items-center justify-center font-black', grade.failed ? 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400' : 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400']">
                ...
                <div>
                  ...
                  <div class="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">
                    Units: {{ grade.units }} | {{ grade.year }} Year | Semester {{ grade.semester }}
                  </div>
                </div>
              </div>
